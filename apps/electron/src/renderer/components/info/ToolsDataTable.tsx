...
export function ToolsDataTable({
  ...
  const columns: ColumnDef<ToolRow>[] = React.useMemo(() => [
    {
      ...
      header: ({ column }) => <SortableHeader column={column} title={t('sourceInfo.access')} />,
      cell: ({ row }) => (
    ...
    {
      accessorKey: 'name',
      header: ({ column }) => <SortableHeader column={column} title={t('sourceInfo.tool')} />,
      cell: ({ row }) => (
        <div className="p-1.5 pl-2.5">
    ...
    },
    {
      ...
      header: () => <span className="p-1.5 pl-2.5">{t('sourceInfo.description')}</span>,
      cell: ({ row }) => (
  ...
  return (
    <Info_DataTable
      ...
      error={error}
      maxHeight={maxHeight}
      emptyContent={t('sourceInfo.noToolsAvailable')}
      className={className}
    />
  )
}
