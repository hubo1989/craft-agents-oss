...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  const handleShare = React.useCallback(async () => {
    ...
    if (result?.success && result.url) {
      ...
      toast.error(t('toast.failedToShare'), { description: result?.error || 'Unknown error' })
    }
  }, [sessionId, t])
  ...
  const handleUpdateShare = React.useCallback(async () => {
    ...
    if (result?.success) {
      ...
      toast.error(t('toast.failedToUpdateShare'), { description: result?.error })
    }
  }, [sessionId, t])
  ...
  const handleRevokeShare = React.useCallback(async () => {
    ...
    if (result?.success) {
      toast.success(t('toast.sharingStopped'))
    } else {
      toast.error(t('toast.failedToStopSharing'), { description: result?.error })
    }
  }, [sessionId, t])
  ...
  if (!session) {
    ...
    return (
      <div className="h-full flex flex-col">
        <PanelHeader  title={t('emptyState.newConversation')} rightSidebarButton={rightSidebarButton} />
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground">
          <AlertCircle className="h-10 w-10" />
          <p className="text-sm">{t('sessionMenu.sessionNotFound')}</p>
        </div>
      </div>
    )
  }
